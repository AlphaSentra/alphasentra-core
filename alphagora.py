...
def get_momentum_recommendations(bull_bear_score):
    ...
    try:
        ...
        data = yf.download(MOMENTUM_ETFS, start=start_date, end=end_date, interval='1mo', progress=False)['Adj Close']
...
class KalmanPairsTrader:
    ...
    def get_pairs_recommendations(self):
        ...
        try:
            ...
            data = yf.download([self.y_ticker, self.x_ticker], start=start_date, end=end_date, progress=False)['Adj Close']
            data = data.dropna()
            ...
            today_data = yf.download([self.y_ticker, self.x_ticker], period='2d', progress=False)['Adj Close']
